 ...
 * Provides geostatistical interpolation (Kriging) for foot pressure sensor data.
 *
 * Usage:
 *   import kriging from '../../utils/kriging';
 *   const variogram = kriging.train(t, x, y, 'exponential', 0, 100);
 *   const predicted = kriging.predict(xCoord, yCoord, variogram);
 */

// --- Array helper functions (replacing Array.prototype extensions) ---
...
function arrayRep(value, n) {
  return Array.apply(null, new Array(n)).map(Number.prototype.valueOf, value);
}

function arrayPip(polygon, x, y) {
  var i,
    j,
    c = false;
...
kriging.train = function (t, x, y, model, sigma2, alpha) {
  ...
  var lag = arrayRep(0, lags);
  var semi = arrayRep(0, lags);
  if (lags < 30) {
    for (l = 0; l < lags; l++) {
      lag[l] = distance[l][0];
      semi[l] = distance[l][1];
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = a[0]; j <= a[1]; j++)
      for (k = b[0]; k <= b[1]; k++) {
        ...
        if (arrayPip(polygons[i], xtarget, ytarget))
          A[j][k] = kriging.predict(xtarget, ytarget, variogram);
      }
  }
  A.xlim = xlim;
  A.ylim = ylim;
  A.zlim = [arrayMin(variogram.t), arrayMax(variogram.t)];
  A.width = width;
  return A;
};
