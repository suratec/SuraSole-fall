...
class MedicalNotes extends Component {
    ...
    fetchMedicalRecord = async () => {
        ...
        try {
            ...
            // Create movement disorders object
            const md = {
                ...
                movementDisorderNone: !!data.movement_disorder_none,
            };
            ...
            const mdKeys = ['ankleLeft', 'ankleRight', 'kneeLeft', 'kneeRight', 'hipLeft', 'hipRight'];
            ...
            // If no movement disorders are checked and movementDisorderNone is not explicitly true, set it to true
            if (mdKeys.every(k => !md[k]) && !md.movementDisorderNone) {
                md.movementDisorderNone = true;
            }

            this.setState({
                medicalConditions: mc,
                walkingAids: wa,
                movementDisorders: md
            });
        } catch (error) {
            Alert.alert(
    ...
    validateForm = () => {
        ...
        // If no medical conditions are selected and dyslipidemiaNone is not selected, auto-select it
        ...
        // If dyslipidemiaNone is selected, skip medical condition suboption validation
        const skipMedicalValidation = medicalConditions.dyslipidemiaNone || !anyMedicalChecked;

        // Validate medical condition suboptions (only if not skipping)
        if (!skipMedicalValidation) {
        ...
        // Walking aids validation (only if walkingAidNone is not selected)
        if (!walkingAids.walkingAidNone) {
            // Validate cane suboptions
            if (walkingAids.cane) {
                if (!walkingAids.caneSide) {
                    Alert.alert(getLocalizedText(this.props.lang, Lang.alertErrorTitle), 'Please select side for cane.');
                    return false;
                }
                if (!walkingAids.caneDuration) {
                    Alert.alert(getLocalizedText(this.props.lang, Lang.alertErrorTitle), 'Please select duration for cane.');
                    return false;
                }
            }

            // Validate crutch suboptions
            if (walkingAids.crutch) {
                if (!walkingAids.crutchSide) {
                    Alert.alert(getLocalizedText(this.props.lang, Lang.alertErrorTitle), 'Please select side for crutch.');
                    return false;
                }
                if (!walkingAids.crutchDuration) {
                    Alert.alert(getLocalizedText(this.props.lang, Lang.alertErrorTitle), 'Please select duration for crutch.');
                    return false;
                }
            }

            // Validate walker suboptions
            if (walkingAids.walker && !walkingAids.walkerDuration) {
                Alert.alert(getLocalizedText(this.props.lang, Lang.alertErrorTitle), 'Please select duration for walker.');
                return false;
            }

            // Validate wheelchair suboptions
            if (walkingAids.wheelchair && !walkingAids.wheelchairDuration) {
                Alert.alert(getLocalizedText(this.props.lang, Lang.alertErrorTitle), 'Please select duration for wheelchair.');
                return false;
            }
        }
    ...
    saveMedicalRecord = async () => {
        ...
        const movementDisorderNone = !movementChecked || !!movementDisorders.movementDisorderNone;
    ...
    handleMainOptionToggle = (category, mainKey, subKeys = []) => {
        this.setState((prevState) => {
            ...
            if (!prevState[category][mainKey]) {
                ...
                if (category === 'movementDisorders') {
                    newState[category].movementDisorderNone = false;
                }
            }
    ...
    renderWalkingAid = (title, mainKey, subOptions = []) => {
        ...
        // Group options for cane and crutch - separate side and duration options
        let groupedOptions = subOptions;
        if (mainKey === 'cane' || mainKey === 'crutch') {
            const sideOptions = subOptions.filter(opt => opt.group === 'side');
            const durationOptions = subOptions.filter(opt => opt.group === 'duration');
            groupedOptions = { side: sideOptions, duration: durationOptions };
        }
        ...
        return (
            <View style={styles.conditionContainer}>
                ...
                {isChecked && subOptions.length > 0 && (
                    <View style={styles.subOptionsContainer}>
                        {(mainKey === 'cane' || mainKey === 'crutch') ? (
                            <>
                                {/* Side options in first row */}
                                <View style={styles.subOptionRow}>
                                    {groupedOptions.side.map((option, index) => {
                                ...
                                {/* Duration options in second row */}
                                <View style={styles.subOptionRow}>
                                    {groupedOptions.duration.map((option, index) => {
                                        const isSelected = walkingAids[option.key] === option.value;
                                        return (
                                            <TouchableOpacity
                                                key={index}
                            ...
                            // Regular single row layout for walker and wheelchair
                            <View style={styles.subOptionRow}>
                                {subOptions.map((option, index) => {
                                    const isSelected = walkingAids[option.key] === option.value;
                                    return (
                    ...
                    <View style={[styles.page, { width }]}>
                        <ScrollView contentContainerStyle={styles.pageContent}>
                            <View style={styles.card}>
                                ...
                                {/* Medical Condition None main checkbox: clears all MC */}
                                <View style={styles.conditionContainer}>
                                    <View style={styles.conditionMainRow}>
                                        <TouchableOpacity
                        ...
                        <ScrollView contentContainerStyle={styles.pageContent}>
                            <View style={styles.card}>
                                ...
                                {this.renderWalkingAid(
                                    ...
                                    'crutch',
                                    [
                                        { label: getLocalizedText(this.props.lang, Lang.leftSide) || 'Left', key: 'crutchSide', value: 'left', group: 'side' },
                                        { label: getLocalizedText(this.props.lang, Lang.rightSide) || 'Right', key: 'crutchSide', value: 'right', group: 'side' },
                                        { label: getLocalizedText(this.props.lang, Lang.both) || 'Both', key: 'crutchSide', value: 'both', group: 'side' },
                                        { label: getLocalizedText(this.props.lang, Lang.lessThanOneYear) || '<1yr', key: 'crutchDuration', value: '<1yr', group: 'duration' },
                                        { label: getLocalizedText(this.props.lang, Lang.moreThanOneYear) || '>1yr', key: 'crutchDuration', value: '>1yr', group: 'duration' },
                                    ]
                                )}
                                {this.renderWalkingAid(
                                ...
                                {this.renderMovementDisorderRow(
                                    getLocalizedText(this.props.lang, Lang.ankle) || 'Ankle',
                                    'ankleLeft',
                                    'ankleRight'
                                )}
                                {this.renderMovementDisorderRow(
                                    getLocalizedText(this.props.lang, Lang.knee) || 'Knee',
                                    'kneeLeft',
                                    'kneeRight'
                                )}
                                {this.renderMovementDisorderRow(
                                    getLocalizedText(this.props.lang, Lang.hip) || 'Hip',
                                    'hipLeft',
                                    'hipRight'
                                )}

                                {/* Movement Disorder None checkbox - positioned on right like other movement disorders */}
                                {this.renderMovementDisorderNone()}
