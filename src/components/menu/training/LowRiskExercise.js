...
export default function LowRiskExercise({ navigation }) {
    ...
    const handleExercisePress = (exerciseName) => {
        console.log('🔗 LowRiskExercise - Exercise clicked:', exerciseName);
        const matchedExercise = findExerciseInAPI(exerciseName);
        console.log('🔗 LowRiskExercise - Matched exercise:', matchedExercise);
        ...
        if (matchedExercise) {
            ...
            console.log('🔗 LowRiskExercise - Navigating with data:', selectedExerciseData);

            navigation.navigate('ExerciseWorkOut', {
                selectedExercise: selectedExerciseData
            });
        } else {
            console.log('❌ LowRiskExercise - Exercise not found in API');
            Alert.alert(
    ...
    const renderMondayContent = () => (
        <ScrollView style={styles.pageContainer} showsVerticalScrollIndicator={false}>
            ...
            {/* Walk */}
            <View style={styles.section}>
                <Text style={styles.sectionTitle}>🚶‍♀️ {getLocalizedText(lang, langTraining.monday.walk.title)} (20 min)</Text>
                <Text style={styles.simpleText}>• {getLocalizedText(lang, langTraining.monday.walk.instruction)}</Text>
                <Text style={styles.simpleText}>• {getLocalizedText(lang, langTraining.monday.walk.pace)}</Text>
                <Text style={styles.simpleText}>• {getLocalizedText(lang, langTraining.monday.walk.breaks)}</Text>
            </View>
    ...
    const renderWednesdayContent = () => (
        <ScrollView style={styles.pageContainer} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.section}>
                ...
                <Text style={styles.simpleText}>• {getLocalizedText(lang, langTraining.wednesday.warmUp)}</Text>
            </View>

            {/* Strength Exercises */}
            <View style={styles.section}>
                <Text style={styles.sectionTitle}>💪 {getLocalizedText(lang, langTraining.wednesday.strength.title)}</Text>
                <Text style={styles.subText}>15 reps × 3 sets each</Text>

                <View style={styles.exerciseGroup}>
                    <ExerciseItem exerciseName="Seated Dead Bug" isTitle={true}>
                        🪑 Seated Dead Bug
                    </ExerciseItem>
                    <Text style={styles.instructionText}>{getLocalizedText(lang, langTraining.wednesday.strength.seatedDeadBug)}</Text>

                    <ExerciseItem exerciseName="Seated Side Bends" isTitle={true}>
                        ↔️ Seated Side Bends
                    </ExerciseItem>
                    <Text style={styles.instructionText}>{getLocalizedText(lang, langTraining.wednesday.strength.seatedSideBends)}</Text>

                    <ExerciseItem exerciseName="Seated Forward Roll-Ups" isTitle={true}>
                        ↘️ Seated Forward Roll-Ups
                    </ExerciseItem>
                    <Text style={styles.instructionText}>{getLocalizedText(lang, langTraining.wednesday.strength.seatedForwardRollUps)}</Text>

                    <ExerciseItem exerciseName="Wood Chops" isTitle={true}>
                        🪓 Wood Chops
                    </ExerciseItem>
                    <Text style={styles.instructionText}>{getLocalizedText(lang, langTraining.wednesday.strength.woodChops)}</Text>

                    <ExerciseItem exerciseName="Wall Planks" isTitle={true}>
                        🧱 Wall Planks (15s hold)
                    </ExerciseItem>
                    <Text style={styles.instructionText}>{getLocalizedText(lang, langTraining.wednesday.strength.wallPlank)}</Text>
                </View>
            </View>
    ...
    const renderFridayContent = () => (
        <ScrollView style={styles.pageContainer} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.section}>
                <Text style={styles.sectionTitle}>🔸 {getLocalizedText(lang, langTraining.warmUp.title)} (5 min)</Text>
                <Text style={styles.simpleText}>• {getLocalizedText(lang, langTraining.friday.warmUp)}</Text>
            </View>

            {/* Balance Exercises */}
            <View style={styles.section}>
                <Text style={styles.sectionTitle}>⚖️ {getLocalizedText(lang, langTraining.friday.balance.title)}</Text>
                <Text style={styles.subText}>15 reps × 3 sets each</Text>
                ...
                <View style={styles.exerciseGroup}>
                    ...
                    <Text style={styles.instructionText}>{getLocalizedText(lang, langTraining.friday.balance.singleLimbStance)}</Text>

                    <ExerciseItem exerciseName="Rock the Boat" isTitle={true}>
                        🚤 Rock the Boat
                    </ExerciseItem>
                    <Text style={styles.instructionText}>{getLocalizedText(lang, langTraining.friday.balance.rockTheBoat)}</Text>

                    <ExerciseItem exerciseName="Back Leg Raises" isTitle={true}>
                        🔙 Back Leg Raises
                    </ExerciseItem>
                    <Text style={styles.instructionText}>{getLocalizedText(lang, langTraining.friday.balance.backLegRaises)}</Text>

                    <ExerciseItem exerciseName="Side Leg Raise" isTitle={true}>
                        ➡️ Side Leg Raises
                    </ExerciseItem>
                    <Text style={styles.instructionText}>{getLocalizedText(lang, langTraining.friday.balance.sideLegRaises)}</Text>

                    <ExerciseItem exerciseName="Toe Lifts" isTitle={true}>
                        📏 Toe Lifts
                    </ExerciseItem>
                    <Text style={styles.instructionText}>{getLocalizedText(lang, langTraining.friday.balance.toeLifts)}</Text>

                    <ExerciseItem exerciseName="Wall Pushups" isTitle={true}>
                        🧱 Wall Push-Ups
                    </ExerciseItem>
                    <Text style={styles.instructionText}>{getLocalizedText(lang, langTraining.friday.balance.wallPushUps)}</Text>
                </View>
            </View>
    ...
    return (
        <View style={styles.container}>
            <HeaderFix
                ...
                title={getLocalizedText(lang, langTraining.lowRiskProgram)}
            />
