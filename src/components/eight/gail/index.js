...
/** ──────────────────────────────────────────────────────────────────────────
 *  Shoe-size parser: extracts "9", "10", "10.5" that appear just before L/R
 *  Examples: "SuraSole_10L", "Device-9R", "Model 10.5L"
 *  Returns a string (e.g., "10.5") or null if not present.
 *  ────────────────────────────────────────────────────────────────────────── */
...
class index extends Component {
    ...
    state = {
        ...
        isConnected: true,
        peripherals: new Map(),
        shoeSize: null, // ← store detected shoe size
    };
                ...
                BleManager.connect(peripheral.id)
                    .then(() => {
                        ...
                        if (peripheral.name[peripheral.name.length - 1] === 'L') {
                            ...
                            // detect shoe size from name and set once
                    ...
                    RNFS.readFile(r.path)
                        .then(text => {
                            ...
                            var content = {
                                ...
                                shoe_size: this.state.shoeSize, // ← include shoe size
                            };
                            console.log(content);
                            fetch(`${API}/addjson`, {
                                method: 'POST',
